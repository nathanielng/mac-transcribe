...
- `SettingsView` — a separate window (recordings folder picker; transcription
  backend segmented control — local MLX-Whisper model field, or Amazon
  Transcribe with S3 bucket/region/profile/max-speakers fields and a
  "Suggest…" button that shells out to `aws sts get-caller-identity` to
  propose a bucket name from your AWS account ID (read-only — never creates
  the bucket itself); outline backend segmented control with a curated model
  picker for each — GLM-5 (default) / Sonnet 5 / DeepSeek V3.2 /
  Qwen3-VL-235B for Bedrock, Qwen3.5 4B/9B/Qwen3.8 27B for local MLX — plus
  free-text "Custom…" for either; a toggle for
